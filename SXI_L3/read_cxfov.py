...
class read_cxfov():
    ...
    def rebin_arrays(self):
        ...
        self.data = {}  
        self.data['CTSMAP'] = self.rebin_sum(self.mapL3.data['CTSMAP'], self.new_shape) 
        self.data['XBMAP'] = self.rebin_sum(self.mapL3.data['XBMAP'], self.new_shape) 
        self.data['PSMAP'] = self.rebin_sum(self.mapL3.data['PSMAP'], self.new_shape) 
        self.data['PBMAP'] = self.rebin_sum(self.mapL3.data['PBMAP'], self.new_shape) 
        self.data['CLMAP'] = self.rebin_sum(self.mapL3.data['CLMAP'], self.new_shape) 
        self.data['SPMAP'] = self.rebin_sum(self.mapL3.data['SPMAP'], self.new_shape) 
        self.data['VIGMAP'] = self.rebin_mean(self.mapL3.data['VIGMAP'], self.new_shape) 
    ...
    def plot_raw_extension_vs_binned(self, ext = 'CTSMAP', cmap='lundi', vmin=0, vmax=10):
        ...
        pos_title = 'SMILE: ({:.2f},{:.2f},{:.2f})'.format(*self.mapL3.smile_loc) 
        aim_title = 'AIM: ({:.2f},{:.2f},{:.2f})'.format(*self.mapL3.target_loc) 
