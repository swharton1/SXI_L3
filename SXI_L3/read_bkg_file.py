#This script reads the bkg file. 
...
class read_bkg_file():
    ...
    def __init__(self, folder='L3_20260317T0240-20260317T0241/', filename='SMILE_SXI_L3_SCIM15-SCI-BKG_20260317T0240-20260317T0241_V01.fits'):
        ...
        with fits.open(self.fullname) as hdul: 
            ...
            #Now data. 
            self.data = {} 
            self.data['BKGMAP'] = self.hdul['PRIMARY'].data 
    ...
    def plot_bkg_extensions(self, cmap='lundi', vmin=0, vmax=2, save=True):
        ...
        fig.subplots_adjust(top=0.90, wspace=0.5)
        ...
        #Make the axis. 
        make_image_axes.make_image_axes(ax1, self.data['BKGMAP'], self.xdeg_min, self.ydeg_min, self.n_pixels, self.m_pixels, cmap=cmap, vmin=vmin, vmax=vmax, cbar_title='')
        ax1.set_title('BKGMAP'+'\n')
                       
        #Make the axis. 
        make_image_axes.make_image_axes(ax2, self.data['XBMAP'], self.xdeg_min, self.ydeg_min, self.n_pixels, self.m_pixels, cmap=cmap, vmin=vmin, vmax=vmax, cbar_title='', ylabel=False)
        ax2.set_title('XBMAP'+'\n')
        
        #Make the axis. 
        make_image_axes.make_image_axes(ax3, self.data['PSMAP'], self.xdeg_min, self.ydeg_min, self.n_pixels, self.m_pixels, cmap=cmap, vmin=vmin, vmax=vmax, cbar_title='Counts/pixel', ylabel=False)
        ax3.set_title('PSMAP'+'\n')
        
        #Make the axis. 
        make_image_axes.make_image_axes(ax4, self.data['PBMAP'], self.xdeg_min, self.ydeg_min, self.n_pixels, self.m_pixels, cmap=cmap, vmin=vmin, vmax=vmax, cbar_title='')
        ax4.set_title('PBMAP'+'\n')
        
        #Make the axis. 
        make_image_axes.make_image_axes(ax5, self.data['CLMAP'], self.xdeg_min, self.ydeg_min, self.n_pixels, self.m_pixels, cmap=cmap, vmin=vmin, vmax=vmax, cbar_title='', ylabel=False)
        ax5.set_title('CLMAP'+'\n')
        
        #Make the axis. 
        make_image_axes.make_image_axes(ax6, self.data['SPMAP'], self.xdeg_min, self.ydeg_min, self.n_pixels, self.m_pixels, cmap=cmap, vmin=vmin, vmax=vmax, cbar_title='Counts/pixel', ylabel=False)
        ax6.set_title('SPMAP'+'\n')
